...
1. Call `revit.status` once. If it fails, use `$revit-mcp-cowork:diagnose-revit`;
   do not install or rebuild Revit MCP Next. After hook trust, the successful
   call registers the private `PLUGIN_DATA` location.
2. Run the plugin-root `scripts/manage-revit-learning.ps1` with
...
succeed. No installation, checkout, Git, GitHub, PR, or publishing is involved.
New Codex tasks detect user-skill updates automatically; if an update is not
visible, restart Codex.
